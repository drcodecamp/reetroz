...
Finishes by rebuilding the site catalog (scripts/build_pages.py) so the flags update.

Usage:
  python scripts/publish_all.py --publication cgw            # everything not rendered yet
  python scripts/publish_all.py --publication cgw --workers 8
  python scripts/publish_all.py --publication cgw --no-upload
"""
...
def main() -> int:
    ...
    pending = [i for i in issues if not (PAGES_DIR / i["id"] / "manifest.json").exists() and resolve_pdf(i["files"]["pdf"]).exists()]
